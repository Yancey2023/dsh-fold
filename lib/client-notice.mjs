...
var NoticeNodeWrapper = React4.memo(function NoticeNodeWrapper2(props) {
  ...
  if (!NOTICE_KINDS.has(node.kind)) return React4.createElement(FoldedSeat2, null);
  const official = officialNodeEntry(node.kind);
  const officialContent = official === void 0 || official.component == null ? null : React4.createElement(official.component, props);
  if (officialContent === null) return React4.createElement(FoldedSeat2, null);
  const group = singleGroup(node);
  const bar = React4.createElement(GroupBar, { group, expanded, onToggle: toggle, onKeyDown, t });
  ...
  if (turnInfo !== null && isProcessNode(turnInfo, node.key)) {
    const first = node.key === turnInfo.firstKey;
    if (!turnExpanded) {
      return first ? React4.createElement(TurnFoldBar, { expanded: false, onToggle: turnToggle, onKeyDown: turnKeyDown, t }) : React4.createElement(FoldedSeat2, null);
    }
    return React4.createElement(
      ...
      React4.createElement(
        ...
        bar,
        expanded ? React4.createElement("div", { className: "dshToolGroupItems" }, officialContent) : null
      )
    );
  }
  return React4.createElement(
