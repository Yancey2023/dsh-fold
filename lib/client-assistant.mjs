...
var ToolCallGroupView = React4.memo(function ToolCallGroupView2(props) {
  ...
  const group = React4.useMemo(() => groupOf(chat, node.key), [chat, node]);
...
var AssistantNodeWrapper = React5.memo(function AssistantNodeWrapper2(props) {
  ...
  const { chat, hasMore, loadingOlder } = useSnapshotFace(props);
  const productFoldActive = props.turnProcess?.foldable === true;
  const group = React5.useMemo(() => isTransparentAssistant(node) ? groupOf(chat, node.key) : null, [chat, node]);
  const turnInfo = React5.useMemo(
    () => productFoldActive ? null : turnProcessOf(chat, node.key),
    [chat, node, productFoldActive]
  );
  ...
  const thinkContent = group !== null && isGroupLeader(group, node.key) ? React5.createElement(
    ...
    expanded ? React5.createElement(GroupItems, { group, t, conversationT: groupConversationT }) : null
  ) : null;
  let output;
