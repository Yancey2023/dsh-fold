...
// ---------------------------------------------------------------------------
{
  ...
  const s = snapshot(
    ['c1', 'u1', 'aTh', 't1', 'ctx1', 'cmd1', 'mr1', 'aSum'],
    [
...
// ---------------------------------------------------------------------------
// Current channels: the adapter's chat face carries the closure map at
// `turnEnds` (flattened from `chat.legacy.turnEnds`) — the direct
// turnProcessOf input shape — and turn locations expose `turn.status`.
// ---------------------------------------------------------------------------

// Chat face with the normalized turnEnds closure map.
{
  const s = {
    order: ['t1', 'aSum'],
  ...
  const info = turnProcessOf(s, 't1')
  assert.ok(info, 'normalized chat face folds')
  assert.equal(info.summaryKey, 'aSum')
  assert.deepEqual([...info.keys], ['t1'], 'only the tool call folds')
}

// Turn-location boundary signal alone (no turnEnds map on the face).
