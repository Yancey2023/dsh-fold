...
// ---------------------------------------------------------------------------
{
  ...
  assert.equal(bubble[0].props['data-clamped'], '', 'bubble must be clamped by default')
  assert.equal(textOf(bubble[0]), 'hello')
...
// ---------------------------------------------------------------------------
{
  ...
  assert.equal(bubble.length, 1)
  assert.equal(textOf(bubble[0]), input, 'chip decoration must preserve the full text')
...
// ---------------------------------------------------------------------------
{
  ...
  const toggle = findAll(json, byClass('dshUserFoldToggle'))
  const bubble = findAll(json, byClass('dshUserBubble'))
  assert.equal(bubble[0].props['data-clamped'], '')
  assert.equal(textOf(toggle[0]), '展开')
  ...
  json = root.toJSON()
  const bubbleAfter = findAll(json, byClass('dshUserBubble'))
  const toggleAfter = findAll(json, byClass('dshUserFoldToggle'))
  assert.equal(bubbleAfter[0].props['data-clamped'], undefined, 'expanded bubble is not clamped')
  ...
  json = root.toJSON()
  const bubbleCollapsed = findAll(json, byClass('dshUserBubble'))
  assert.equal(bubbleCollapsed[0].props['data-clamped'], '', 'collapse restores the clamp')
...
// ---------------------------------------------------------------------------
{
  ...
  const json = root.toJSON()
  assert.equal(findAll(json, byClass('dshUserBubble')).length, 0)
...
// ---------------------------------------------------------------------------
{
  ...
  const bubble = findAll(json, byClass('dshUserBubble'))
  assert.equal(bubble.length, 1)
  assert.equal(textOf(bubble[0]), 'plain string')
  root.unmount()
}
