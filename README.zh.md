...
使用同一范围，无论宿主走哪个通道都解析成功。最新的 alpha
`0.1.6-alpha.2` 为 `SlotCore` 增加了可复用的 Factory 子系统；overlay 依赖的
`register` / `releaseEntry` 子槽声明契约未变——`test/overlay.test.mjs`
直接跑真实包（发布版与压缩版两种形态）。更早版本已不在支持范围——
针对它们的兼容层已全部移除（`useSession` 携带聊天快照的适配、命名空间探测、
`status`/`final` 兜底判断、以及插件自带的轮次级大折叠——该折叠由产品负责）。
本序列共用同一套 chat-node seat kit：`useChat` 返回聊天目标
...
`tool.call.toolview` owner `loadImage` 货币，`0.1.6-alpha.2` 原样保留）——封闭在
